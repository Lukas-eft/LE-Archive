# LE Archive
**Developed by Lukas Eifertinger**

LE Archive is my personal, JavaScript-based wishlist website. It uses local storage and modern background blur filter elements.
<img width="1909" height="1079" alt="Screenshot 2026-02-14 213708" src="https://github.com/user-attachments/assets/552de6bd-d742-4c3f-90dd-966601590e21" />


###  Key Functionalities

* **Responsive Scaling**: The site dynamically fits to the size of the mobile or desktop device.
* **Liquid Glass UI**: A modern main menu featuring blur effects (`backdrop-filter`) and interactive elements.
* **Saving Data**: Your wishes are saved locally in the browser (`localStorage`).

###  Technical Overview



#### File Structure
* `index.html`: The structure of the site and UI layers aswell as all styling integrated.
* `main.js`: Core functionality.

### CopyRight
For any Copyright reasons or concerns feel free to contact me.
